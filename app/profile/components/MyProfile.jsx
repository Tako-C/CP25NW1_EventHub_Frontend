...
export default function ProfilePage({
  ...
  const handleSave = async () => {
    ...
    const cleanPayload = {
      ...
      job: updateProfile.job?.id
        ? { id: updateProfile.job.id, jobNameTh: updateProfile.job.jobNameTh, jobNameEn: updateProfile.job.jobNameEn }
        : null,
      country: updateProfile.country?.id
        ? { id: updateProfile.country.id, countryNameTh: updateProfile.country.countryNameTh, countryNameEn: updateProfile.country.countryNameEn }
        : null,
      city: updateProfile.city?.id
        ? { id: updateProfile.city.id, cityNameTh: updateProfile.city.cityNameTh, cityNameEn: updateProfile.city.cityNameEn }
        : null,
    };
  ...
  return (
    <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6 lg:p-8">
      ...
      {showIncompleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full">
            ...
            <p className="text-sm text-gray-500 mb-3 ml-[52px]">ยังไม่ได้ระบุข้อมูลต่อไปนี้:</p>
      ...
      {/* Yellow warning banner — ซ่อนระหว่างโหลด */}
      {!isLoading && !isEditing && getMissingFields(profile).length > 0 && (
      ...
      <div className="mb-6 md:mb-8 border-b border-gray-100 pb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl md:text-2xl font-bold text-gray-800">ข้อมูลบัญชีของฉัน</h2>
          <p className="text-gray-500 text-sm mt-1">จัดการและแก้ไขข้อมูลส่วนตัวของคุณ</p>
        </div>
        {!isEditing && (
          <button
      ...
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-10">
          ...
          <div className="text-center">
            {isLoading ? (
              <>
                ...
                <div className="h-5 w-20 bg-gray-100 rounded-full animate-pulse mx-auto mt-2" />
              </>
            ) : (
              <>
        ...
        <div className="flex-1 w-full">
          {isLoading ? (
            /* Skeleton */
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-x-8 md:gap-y-6 animate-pulse">
              {[...Array(8)].map((_, i) => (
                <div key={i} className={i === 6 ? "md:col-span-2" : ""}>
                  <div className="h-3.5 w-16 bg-gray-200 rounded mb-2" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-x-8 md:gap-y-6">
              ...
              <div className="flex flex-col">
                <label className="block text-sm font-medium text-gray-700 mb-1.5 ml-1">เพศ</label>
                {isEditing ? (
                  ...
                  <div className="w-full pl-10 pr-4 py-2.5 bg-white border-b border-gray-200 text-gray-800 font-medium flex items-center text-sm md:text-base min-h-[44px] relative">
                    ...
                    {genderOptions.find((g) => g.id === updateProfile.gender)?.label || "ไม่ระบุ"}
                  </div>
                )}
              </div>
              ...
              {/* อายุ */}
              <div className="flex flex-col">
                <label className="block text-sm font-medium text-gray-700 mb-1.5 ml-1">อายุ</label>
                <div className="relative">
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">
                    <Calendar size={18} />
                  </div>
              ...
              <div>
                ...
                />
                {!isLoading && !isEditing && !profile?.job?.id && (
              ...
              <div>
                ...
                />
                {!isLoading && !isEditing && !profile?.country?.id && (
              ...
              <div>
                ...
                />
                {!isLoading && !isEditing && !profile?.city?.id && (
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1.5 ml-1">ที่อยู่</label>
                {isEditing ? (
                  <textarea
              ...
              {/* Save / Cancel */}
              {isEditing && (
                <div className="md:col-span-2 flex flex-col-reverse sm:flex-row justify-end gap-3 mt-2 pt-6 border-t border-gray-100">
                  <button
        ...
        {isEditing ? (
          <div className="relative">
            ...
            <input
              ...
              maxLength={maxLength}
              className={`w-full ${icon ? "pl-10" : "pl-4"} pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all text-gray-700 text-sm md:text-base ${disabled ? "opacity-60 cursor-not-allowed" : ""}`}
            />
          </div>
        ) : (
          <div className={`w-full ${icon ? "pl-10" : "pl-4"} pr-4 py-2.5 bg-white border-b border-gray-200 text-gray-800 font-medium flex items-center text-sm md:text-base min-h-[44px] relative`}>
            {icon && (
              <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">
                {icon}
              </div>
...
function SelectField({
  ...
  const getDisplayName = (val) => {
    ...
    const found = options.find((opt) => opt.id === val || opt.id === Number(val));
    if (found) return found.jobNameTh || found.countryNameTh || found.cityNameTh;
    return val;
  };
        ...
        {isEditing ? (
          <div className="relative">
            ...
            <select
              ...
              onChange={(e) => onChange(e.target.value)}
              className={`w-full ${icon ? "pl-10" : "pl-4"} pr-10 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all text-gray-700 text-sm md:text-base appearance-none`}
            >
          ...
          <div className={`w-full ${icon ? "pl-10" : "pl-4"} pr-4 py-2.5 bg-white border-b border-gray-200 text-gray-800 font-medium flex items-center text-sm md:text-base min-h-[44px] relative`}>
            ...
            <span className="truncate w-full block">{getDisplayName(value)}</span>
          </div>
        )}
      </div>
    </div>
