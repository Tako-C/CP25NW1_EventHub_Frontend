...
import { ClipboardList, Search, X, Tag } from "lucide-react";
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 border-4 border-gray-200 border-t-indigo-500 rounded-full animate-spin mx-auto mb-3" />
      ...
      <section className="max-w-7xl mx-auto py-6 px-4 md:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-5 mt-6">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">My Surveys</h1>
            <p className="text-sm text-gray-400 mt-0.5">
              จัดการแบบสำรวจสำหรับกิจกรรมที่คุณเป็นผู้จัด
            </p>
          </div>
          ...
          {eventData.length > 0 && (
            <div className="relative w-full sm:w-72">
              ...
              <input
                ...
                className="w-full pl-9 pr-8 py-2.5 text-sm bg-white border border-gray-200 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-transparent transition"
              />
              {search && (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((event) => (
              <SurveyEventCard
                key={event.id}
                event={event}
                onClick={() => router.push(`/organizer/survey/${event.id}`)}
              />
            ))}
          </div>
        )}
