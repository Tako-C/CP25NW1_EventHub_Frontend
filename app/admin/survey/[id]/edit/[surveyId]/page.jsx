...
export default function EditAdminSurveyPage() {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (q.questionType === "SINGLE" || q.questionType === "MULTIPLE") {
        if (!q.choices || q.choices.length < 3)
          return showNotification(`คำถามที่ ${i + 1} ต้องมีอย่างน้อย 3 ตัวเลือก`, true);
        if (q.choices.length > 10)
          return showNotification(`คำถามที่ ${i + 1} มีตัวเลือกได้ไม่เกิน 10 ข้อ`, true);
        if (q.choices.some((choice) => !choice.trim()))
          return showNotification(`กรุณากรอกข้อความในทุกตัวเลือกของคำถามที่ ${i + 1}`, true);
      }
    }
  ...
  const resolvedSurveyType = surveyType?.startsWith("PRE") ? "pre" : "post";
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
        {!showPreview ? (
          <>
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs text-blue-700">
                    <div className="bg-white rounded-lg p-3 border border-blue-100">
                      ...
                      <div>
                        แนะนำ <span className="font-bold text-blue-600">5–8 ข้อ</span> เพื่อให้ผู้ตอบไม่รู้สึกเบื่อ
                      </div>
                    ...
                    <div className="bg-white rounded-lg p-3 border border-blue-100">
                      ...
                      <div>
                        แนะนำ <span className="font-bold text-blue-600">3–5 ตัวเลือก</span> ต่อคำถาม
                      </div>
                    ...
                    <div className="bg-white rounded-lg p-3 border border-blue-100">
                      ...
                      {resolvedSurveyType === "pre" ? (
                        ...
                        <ul className="space-y-0.5">
                          ...
                          <li>
                            • ข้อเสนอแนะ <span className="text-red-500">(ต้องมี)</span>
                          </li>
                        </ul>
                      )}
                    </div>
            ...
            {/* ปุ่มเพิ่มคำถาม: เต็มรูปแบบ < 5, subdued 5–9, ซ่อนเมื่อครบ 10 */}
            {questions.length < 5 && (
              <button
                onClick={handleAddQuestion}
                className="w-full py-4 border-2 border-dashed border-gray-300 rounded-xl text-gray-600 hover:border-purple-500 hover:text-purple-600 hover:bg-purple-50 flex items-center justify-center gap-2 font-semibold transition-all"
              >
            ...
            {questions.length >= 5 && questions.length < 10 && (
              <button
                onClick={handleAddQuestion}
                className="w-full py-3 border border-dashed border-gray-200 rounded-xl text-gray-400 hover:border-gray-400 hover:text-gray-500 flex items-center justify-center gap-2 text-sm font-medium transition-all"
              >
