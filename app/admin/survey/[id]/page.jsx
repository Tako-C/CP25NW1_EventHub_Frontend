...
function SurveyTable({ surveys, surveyType, role, eventId, onStatusChange, onDelete, onEdit }) {
        ...
        <Space size={8}>
          ...
          <button
            ...
            style={{
              ...
              background: "#fff7f7", cursor: "pointer", display: "flex",
  ...
  if (!surveys?.length) {
    return (
      <div style={{
        textAlign: "center", padding: "48px 0", color: "#cbd5e1",
...
function RoleSection({ title, icon: Icon, color, accentColor, surveys, surveyType, role, eventId, onStatusChange, onDelete, onEdit, onCreate }) {
  return (
    <div style={{
      borderRadius: 20, border: `1.5px solid ${color}22`,
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            fontSize: 12, fontWeight: 700, boxShadow: `0 4px 12px ${color}40`,
...
export default function EventSurveyManager() {
  ...
  const handleStatusChange = async (surveyId, newStatus, type, role) => {
    try {
      await patchSurvey(surveyId, { status: newStatus });
      setSurveys(prev => ({
        ...prev,
        [type]: {
          ...prev[type],
          [role]: prev[type][role].map(s => s.id === surveyId ? { ...s, status: newStatus } : s),
        },
      }));
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", padding: "40px 32px" }}>
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          background: "#f1f5f9", padding: 6, borderRadius: 16,
          width: "fit-content",
        }}>
          {tabs.map(tab => {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  boxShadow: isActive ? `0 2px 12px ${tab.color}20, 0 1px 3px #0001` : "none",
              ...
              >
                ...
                {tab.label}
                {(() => {
                  const count = (surveys[tab.key]?.visitor?.length ?? 0) + (surveys[tab.key]?.exhibitor?.length ?? 0);
                  return count > 0 ? (
                    <span style={{
        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <RoleSection
              ...
              onEdit={handleEdit}
              onCreate={() => router.push(`/admin/survey/${id}/create?role=visitor&type=${activeTab}`)}
            />
            ...
            <RoleSection
              ...
              onDelete={handleDelete}
              onEdit={handleEdit}
              onCreate={() => router.push(`/admin/survey/${id}/create?role=exhibitor&type=${activeTab}`)}
            />
          </div>
        )}
      </div>
