...
export default function AdminEventDashboard() {
  ...
  const overallSurveyRate = totalSurvey ? (totalSubmitted / totalSurvey) * 100 : 0;
  ...
  const activeTheme = useMemo(() => {
    const source = `${eventMeta.eventName} ${eventMeta.eventType} ${eventMeta.eventDetail}`.toLowerCase();
    if (/(fashion|style|runway|beauty|boutique|catwalk|design)/.test(source)) return EVENT_THEMES.fashion;
    if (/(tech|technology|software|digital|ai|innovation|developer|it|cyber)/.test(source)) return EVENT_THEMES.tech;
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // Admin uses admin/events/:id endpoint
      const eventRes = await getData(`admin/events/${id}`);
      setTitle(eventRes?.data?.eventName);

      const eventTypeText = extractText(eventRes?.data?.eventType) || extractText(eventRes?.data?.type) || "";
      const eventDetailText = extractText(eventRes?.data?.eventDetail) || extractText(eventRes?.data?.description) || "";
      setEventMeta({
      ...
      // All dashboard APIs are the same endpoint structure
      const [
        regRes, checkinRes,
        visitorStatsRes, exhibitorStatsRes,
        visitorStatusRes, exhibitorStatusRes,
        visitorSatisfactionRes, exhibitorSatisfactionRes,
      ...
      if (visitorSatisfactionRes.status === "fulfilled") setVisitorSatisfaction(visitorSatisfactionRes.value?.data ?? []);
      if (exhibitorSatisfactionRes.status === "fulfilled") setExhibitorSatisfaction(exhibitorSatisfactionRes.value?.data ?? []);
      ...
      if (checkInListRes.status === "fulfilled" && Array.isArray(checkInListRes.value?.data)) {
        setParticipant(
          checkInListRes.value.data.map((item, index) => ({ ...item, key: index, no: index + 1 }))
        );
      } else {
        setParticipant([]);
      }
      ...
      <div className="flex justify-between items-start">
        <div>
          <h3 className="font-bold text-gray-800 text-lg flex items-center gap-2">
            <UserOutlined className="text-indigo-500" /> {item.name}
          </h3>
...
// ─── Sub-components (same as organizer) ────────────────────────────────────────
...
function RevealSection({ children, order = 0 }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => entries.forEach((entry) => { if (entry.isIntersecting) { setVisible(true); observer.unobserve(entry.target); } }),
