...
export default function CheckInStaff() {
  ...
  const handleCheckIn = async (visitorData) => {
    ...
    setIsUpdating(true);
    try {
      const result = await userCheckIn("manual/check-in", selectedEventId,userId);
  ...
  const handleSearch = async () => {
    ...
    try {
      const result = await getListUser("manual/search", searchEmail, selectedEventId);
      ...
      <div className="max-w-4xl mx-auto mt-8 px-4">
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          <p className="text-center text-gray-600 mb-8">
            Event : <span className="font-medium text-purple-600">{selectedEventObj ? selectedEventObj.eventName : "Please select an event"}</span>
          </p>
          ...
          <div className="mb-6">
            ...
            >
                <option value="" disabled>-- กรุณาเลือกงานกิจกรรม (Select Event) --</option>
                {events.map((event) => (
          ...
          <div className="mb-6">
            <label className="block text-gray-700 font-medium mb-2">Visitors Search</label>
            ...
            {!selectedEventId && (
                <p className="text-red-500 text-sm mt-2">* กรุณาเลือก Event ด้านบนก่อนเริ่มค้นหา</p>
            )}
          </div>
              ...
              <>
                {visitors.length > 0 ? (
                      ...
                      <div key={index} className="bg-gray-100 rounded-lg p-6">
                        <div className="flex justify-between items-center">
                          <div className="space-y-2">
                              ...
                              <span className="text-gray-900">{visitor.userId}</span>
                            </div>
                            <div className="flex">
                  ...
                  <div className="text-center text-gray-500 py-8">
                    {hasSearched ? "ไม่พบข้อมูลผู้เข้าร่วมงาน (User Not Found)" : "กรุณากรอกข้อมูลเพื่อค้นหา"}
                  </div>
                )}
              </>
            )}
