...
export default function StaffCheckInPage() {
  ...
  const eventNotStarted =
    selectedEvent && new Date(selectedEvent.startDate) > now;
  ...
  useEffect(() => {
    ...
    if (events.length > 0 && paramEventId) {
      if (events.find((e) => e.eventId.toString() === paramEventId))
  ...
  useEffect(() => {
    if (activeTab === "qr")
      setIsScanning(!!selectedEventId && !isEventUnavailable);
  }, [activeTab]);
    ...
    try {
      ...
      if (res?.statusCode === 200 && res.data) {
        ...
        if (
          ...
          showNotification(
            `QR นี้เป็นของ "${event.eventName}" กรุณาเลือกกิจกรรมให้ตรงแล้วสแกนใหม่`,
            true,
          );
          setIsScanning(true);
          return;
        ...
        if (user.imgPath) {
          const clean = user.imgPath.startsWith("/")
            ? user.imgPath.substring(1)
            : user.imgPath;
          getImage(`upload/users/${clean}`)
            .then(setUserImage)
  ...
  const handleConfirmCheckIn = async () => {
    ...
    try {
      ...
      if (res?.statusCode === 200 || res?.message === "Check-in successful") {
        showNotification(
          `Check-in สำเร็จ: ${confirmData.firstName} ${confirmData.lastName}`,
        );
        setConfirmData(null);
        setIsScanning(true);
  ...
  const handleSearch = async () => {
    if (!selectedEventId) {
      showNotification("กรุณาเลือกกิจกรรมก่อน", true);
      return;
    }
    if (!searchQuery.trim()) {
      showNotification("กรุณากรอกข้อมูลเพื่อค้นหา", true);
      return;
    }
    setIsLoading(true);
    setHasSearched(true);
  ...
  const handleCheckIn = async (visitor) => {
    ...
    try {
      ...
      if (result?.statusCode === 200) {
        setVisitors((prev) =>
          prev.map((v) =>
            v.userId === visitor.userId ? { ...v, status: "check_in" } : v,
          ),
        );
        showNotification("Check-in สำเร็จ");
      } else showNotification("Check-in ไม่สำเร็จ", true);
    } catch {
      showNotification("เกิดข้อผิดพลาด กรุณาลองใหม่", true);
    } finally {
      setIsUpdating(false);
    }
  };
          ...
          <div className="bg-white rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl border border-gray-100">
            ...
            <div className="p-5 space-y-3">
              <p className="text-center text-sm text-gray-500">
                ยืนยันการ Check-in สำหรับผู้เข้าร่วมงานคนนี้?
              </p>
              ...
              {!selectedEventId && (
                <div className="bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 text-center">
                  <p className="text-xs text-amber-700 font-medium">
                    ⚠ กรุณาเลือกกิจกรรมก่อนสแกน QR
                  </p>
                </div>
              )}
              {isEventUnavailable && (
                <div
                  className={`border rounded-2xl px-4 py-3 text-center ${eventEnded ? "bg-gray-50 border-gray-200" : "bg-blue-50 border-blue-200"}`}
                >
                  <p
                    className={`text-xs font-medium ${eventEnded ? "text-gray-500" : "text-blue-700"}`}
                  >
              ...
              <div className="bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-sm">
                ...
                <div className="px-5 py-4 flex items-center gap-3 border-b border-gray-50">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      ชี้กล้องไปที่ QR Code บนบัตรผู้เข้าร่วม
                    </p>
                  </div>
                </div>
                ...
                <div className="relative aspect-square bg-gray-900">
                  {!selectedEventId ? (
                    <div className="w-full h-full flex flex-col items-center justify-center gap-4 px-6">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-gray-400 text-xs mt-1">
                          เลือกกิจกรรมด้านบนเพื่อเปิดกล้อง
                        </p>
                      </div>
                    </div>
                  ) : isEventUnavailable ? (
                    <div className="w-full h-full flex flex-col items-center justify-center gap-4 px-6">
                      <div
                        className={`w-16 h-16 rounded-2xl flex items-center justify-center border ${eventEnded ? "bg-gray-500/20 border-gray-500/30" : "bg-blue-500/20 border-blue-500/30"}`}
                      >
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    disabled={
                      isLoading || !selectedEventId || isEventUnavailable
                    }
              ...
              {isLoading ? (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-start gap-3 min-w-0 flex-1">
                            <div
                              className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5 ${checked ? "bg-emerald-100" : "bg-purple-50"}`}
                            >
                              {checked ? (
                                <CheckCircle2
                                  size={18}
                          ...
                          {checked ? (
                            ...
                            <span
                              className={`flex-shrink-0 text-xs font-bold px-3 py-1.5 rounded-full ${eventEnded ? "text-gray-400 bg-gray-100 border border-gray-200" : "text-blue-400 bg-blue-50 border border-blue-200"}`}
                            >
                ...
                <div className="flex flex-col items-center justify-center py-14 gap-3">
                  ...
                  <p className="text-xs text-gray-400">
                    ลองค้นหาด้วยชื่อ, อีเมล หรือเบอร์โทร
                  </p>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center py-16 gap-3">
                  ...
                  <p className="text-sm font-semibold text-gray-600">
                    พร้อมสำหรับการ Check-in
                  </p>
                  <p className="text-xs text-gray-400">
                    ค้นหาชื่อผู้เข้าร่วมด้านบน
                  </p>
                </div>
              )}
            </div>
