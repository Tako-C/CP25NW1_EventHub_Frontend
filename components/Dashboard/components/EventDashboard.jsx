...
export default function EventDashboard({ mode = "organizer" }) {
  ...
  const activeTheme = useMemo(() => {
    ...
    if (/(fashion|style|runway|beauty|boutique|catwalk|design)/.test(src))
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setEventMeta({
        ...
        eventType: extractText(
          eventRes?.data?.eventType ||
            eventRes?.data?.type ||
            eventRes?.data?.eventCategory ||
            "",
        ),
        eventDetail: extractText(
          eventRes?.data?.eventDetail || eventRes?.data?.description || "",
        ),
      });
    ...
    //   console.log(testData)
      ...
      if (visitorSatRes.status === "fulfilled")
        setVisitorSatisfaction(visitorSatRes.value?.data ?? []);
      if (exhibitorSatRes.status === "fulfilled")
        setExhibitorSatisfaction(exhibitorSatRes.value?.data ?? []);
      ...
      <div className="flex justify-between items-start">
        <div>
          <h3 className="font-bold text-gray-800 text-lg flex items-center gap-2">
            <UserOutlined
              className={isAdmin ? "text-indigo-500" : "text-purple-500"}
            />
            {item.name}
          </h3>
      ...
      <div className="relative max-w-7xl mx-auto p-3 md:p-6 lg:p-8 space-y-5 md:space-y-6">
        ...
        <RevealSection order={13}>
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-3 md:gap-4 items-start">
            ...
            <ChartCard
              title="คำแนะนำแยกตาม Keyword & Sentiment"
              className="self-start"
            >
