        ...
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="relative flex-1 max-w-xs">
              <select
                value={questions?.questionType || "TEXT"}
          ...
          <input
            ...
            readOnly={isLockedPost}
            onChange={(e) => onUpdate(index, "question", e.target.value)}
            placeholder={getPlaceholder()}
            className="w-full text-lg font-medium mb-3 px-0 border-0 border-b-2 border-gray-200 focus:border-purple-500 focus:outline-none transition-colors disabled:bg-white"
          />

          {(questions?.questionType === "SINGLE" || questions?.questionType === "MULTIPLE") && (
              ...
              {questions.choices?.map((option, optIndex) => (
                <div key={optIndex} className="flex items-center gap-2">
                  ...
                  {!isLockedPost && (
                    <button
                      onClick={() => {
                        const newOptions = questions.choices.filter((_, i) => i !== optIndex);
                        onUpdate(index, "choices", newOptions);
                      }}
